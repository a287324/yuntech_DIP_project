...
int main(int argc, char *argv[])
{
	...
	int threshold_h = 230, threshold_l = 20;
	
	//開啟檔案 
	open_bmp("1.bmp", R, R, R, width, height);
	
	//Canny_edge_detector 
	Canny_edge_detector(r, Gaussian_var, threshold_h, threshold_l, R, width, height);
	
	//儲存檔案 
	save_bmp("1_new4.bmp", r, r, r, width, height); // for gray images
	printf("Job Finished!\n");
...
void Canny_edge_detector(int result[][MaxBMPSizeY], float var, int threshold_h, int threshold_l, int origin[][MaxBMPSizeY], int width, int height)
{
	...
	for(i = 1; i < (width - 1); i++)
	{
		for(j = 1; j < (height - 1); j++)
		{
			switch (int(img_dir[i][j])) // 這邊寫錯,壓制的結果並不能直接更改結果圖,應該把壓制的結果儲存在另一個表格,並且在後面的double threshold時使用。後面的double threshold的流程也有問題,後面的註解在詳細說明
			{
				case 0:
	...
	// double threshold這部分有寫錯,這邊把振幅圖3值化(強邊緣/弱邊緣/無邊緣)後,再把前面的 nonmaximum suppression的壓制結果反應到三值化,最後才是連接弱邊緣
