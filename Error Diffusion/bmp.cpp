...
// **************************************************
// open_bmp
// Open and load a bitmap image file into memory (3 RGB arrays of MaxBMPSizeX x MaxBMPSizeY).
// **************************************************

void open_bmp(char filename[], int bmp_r[][MaxBMPSizeY], int bmp_g[][MaxBMPSizeY], int bmp_b[][MaxBMPSizeY], int& width, int& height) {
	int r, g, b;
	int x, y;
...
// **************************************************
// save_bmp
// Save a bitmap image file from memory (3 RGB arrays of MaxBMPSizeX x MaxBMPSizeY).
// **************************************************

void save_bmp(char filename[], int bmp_r[][MaxBMPSizeY], int bmp_g[][MaxBMPSizeY], int bmp_b[][MaxBMPSizeY], int& width, int& height)
{
	int r, g, b;
	int x, y;
	
	bmp_tmp.info_header.biWidth = width;
	bmp_tmp.info_header.biHeight = height;
	if(bmp_tmp.info_header.biBitCount == 24){
		bmp_tmp.info_header.biSizeImage =  height * (width * 3 + width % 4);
		bmp_tmp.file_header.bfSize = bmp_tmp.info_header.biSizeImage + bmp_tmp.file_header.bfOffBits;
	}else if(bmp_tmp.info_header.biBitCount == 8){
		if(width % 4 != 0){
			bmp_tmp.info_header.biSizeImage = height * (int(width / 4) + 1) * 4;
		} else {
			bmp_tmp.info_header.biSizeImage = height * width;
		}
		bmp_tmp.file_header.bfSize = bmp_tmp.info_header.biSizeImage + bmp_tmp.file_header.bfOffBits;
	}
	if ((bmp_tmp.image_process_data = (unsigned char*) malloc(bmp_tmp.info_header.biSizeImage)) == NULL) {
		printf("couldn't get memory for file %s.\n",filename);
		exit(1);
	}
	bmp_tmp.bmp_gap = bmp_tmp.info_header.biWidth % 4;
...
void close_bmp()
{
	...
	free(bmp_tmp.image_process_data);
}
...
void save_bmp_image(char filename[], MyBITMAP *bmp)
{
	...
	bmp->file_header.buf[2] = bmp->file_header.bfSize & 0xff;
	bmp->file_header.buf[3] = (bmp->file_header.bfSize >> 8) & 0xff;
	bmp->file_header.buf[4] = (bmp->file_header.bfSize >> 16) & 0xff;
	bmp->file_header.buf[5] = (bmp->file_header.bfSize >> 24) & 0xff;
	fwrite(&(bmp->file_header.buf), sizeof(char), 14, fp);

	bmp->info_header.buf[4] = bmp->info_header.biWidth & 0xff;
	bmp->info_header.buf[5] = (bmp->info_header.biWidth >> 8) & 0xff;
	bmp->info_header.buf[6] = (bmp->info_header.biWidth >> 16) & 0xff;
	bmp->info_header.buf[7] = (bmp->info_header.biWidth >> 24) & 0xff;
	
	bmp->info_header.buf[8] = bmp->info_header.biHeight & 0xff;
	bmp->info_header.buf[9] = (bmp->info_header.biHeight >> 8) & 0xff;
	bmp->info_header.buf[10] = (bmp->info_header.biHeight >> 16) & 0xff;
	bmp->info_header.buf[11] = (bmp->info_header.biHeight >> 24) & 0xff;
	
	bmp->info_header.buf[20] = bmp->info_header.biSizeImage & 0xff;
	bmp->info_header.buf[21] = (bmp->info_header.biSizeImage >> 8) & 0xff;
	bmp->info_header.buf[22] = (bmp->info_header.biSizeImage >> 16) & 0xff;
	bmp->info_header.buf[23] = (bmp->info_header.biSizeImage >> 24) & 0xff;
	...
	if (bmp->info_header.biBitCount == 8) { /* for gray images */
		...
		fwrite(bmp->image_process_data, sizeof(char), bmp->info_header.biSizeImage, fp);
...
inline void set_bmp_pixel(MyBITMAP *bmp, int x, int y, int color_r, int color_g, int color_b)
{
	...
	int offset;
        ...
        if (bmp->info_header.biBitCount == 8) { /* for gray images */
			...
			if(bmp->bmp_gap != 0)
				offset = (y * width + x) + y * (4 - bmp->bmp_gap);
			else
				offset = (y * width + x);

	        *(bmp->image_process_data+offset+0) = (unsigned char)(color_b & 0xff);
	        ...
	        *(bmp->image_process_data+offset+0) = (unsigned char)(color_b & 0xff);
        	*(bmp->image_process_data+offset+1) = (unsigned char)(color_g & 0xff);
        	*(bmp->image_process_data+offset+2) = (unsigned char)(color_r & 0xff);
