...
#define D(U, V) sqrt((U)*(U) + (V)*(V))
#define Butterworth_LPF_FWF(U, V, D0, N) (1.0 / (1 + pow((D((U),(V)) / (D0)) ,(2*N))))
#define Butterworth_HPF_FWF(U, V, D0, N) (1.0 - Butterworth_LPF_FWF(U, V, D0, N))
...
int main(int argc, char *argv[])
{
	...
	//Butterworth_LPF and Butterworth_HPF
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			sp_re_LPF[i][j] = sp_re[i][j] * Butterworth_LPF_FWF(i-mm, j-mm, Cutoff_freq, filter_order);
			sp_im_LPF[i][j] = sp_im[i][j] * Butterworth_LPF_FWF(i-mm, j-mm, Cutoff_freq, filter_order);
			
			sp_re_HPF[i][j] = sp_re[i][j] * Butterworth_HPF_FWF(i-mm, j-mm, Cutoff_freq, filter_order);
			sp_im_HPF[i][j] = sp_im[i][j] * Butterworth_HPF_FWF(i-mm, j-mm, Cutoff_freq, filter_order);
		}
	}
